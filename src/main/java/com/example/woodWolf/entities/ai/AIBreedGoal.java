package com.example.woodWolf.entities.ai;
...
import com.example.woodWolf.entities.BaseWolfEntity;
import com.example.woodWolf.entities.Gender;
...
public class AIBreedGoal extends Goal {
	...
	protected final BaseWolfEntity animal;
	private final Class<? extends BaseWolfEntity> mateClass;
	protected final World world;
	protected BaseWolfEntity targetMate;
	private int spawnBabyDelay;
	private final double moveSpeed;

	public AIBreedGoal(BaseWolfEntity animal, double speedIn) {
		this(animal, speedIn, animal.getClass());
	}

	public AIBreedGoal(BaseWolfEntity p_i47306_1_, double p_i47306_2_, Class<? extends BaseWolfEntity> p_i47306_4_) {
	...
	public boolean shouldExecute() {
		//of dit moet female worden
		if(this.animal.getGender() == Gender.FEMALE) {
			this.targetMate = this.getNearbyMate();
			return this.targetMate != null;
		}
		else if (!this.animal.isInLove()) {
		return false;
		} else {
		this.targetMate = this.getNearbyMate();
		return this.targetMate != null;
	 ...
	 */
	@Nullable
	private BaseWolfEntity getNearbyMate() {
		List<BaseWolfEntity> list = this.world.getTargettableEntitiesWithinAABB(this.mateClass, field_220689_d, this.animal, this.animal.getBoundingBox().grow(8.0D));
		double d0 = Double.MAX_VALUE;
		BaseWolfEntity animalentity = null;

		for(BaseWolfEntity animalentity1 : list) {
