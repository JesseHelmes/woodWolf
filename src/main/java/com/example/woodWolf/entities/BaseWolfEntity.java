...
import com.example.woodWolf.entities.ai.AIBegGoal;
...
public abstract class BaseWolfEntity extends TameableEntity {
	private static final DataParameter<Boolean> BEGGING = EntityDataManager.createKey(BaseWolfEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> COLLAR_COLOR = EntityDataManager.createKey(BaseWolfEntity.class, DataSerializers.VARINT);
	...
	//custom variables van mij
	private Item bone = Items.BONE;
	private Gender gender;

	public BaseWolfEntity(EntityType<? extends BaseWolfEntity> type, World worldIn) {
		super(type, worldIn);
		this.setTamed(false);
	...
	protected void registerGoals() {
		...
		//this.goalSelector.addGoal(3, new BaseWolfEntity.AvoidEntityGoal(this, LlamaEntity.class, 24.0F, 1.5D, 1.5D));
		this.goalSelector.addGoal(4, new LeapAtTargetGoal(this, 0.4F));
		this.goalSelector.addGoal(5, new MeleeAttackGoal(this, 1.0D, true));
		this.goalSelector.addGoal(6, new FollowOwnerGoal(this, 1.0D, 10.0F, 2.0F, false));
		this.goalSelector.addGoal(7, new BreedGoal(this, 1.0D));
		this.goalSelector.addGoal(8, new WaterAvoidingRandomWalkingGoal(this, 1.0D));
		this.goalSelector.addGoal(9, new AIBegGoal(this, 8.0F));
	...
	protected void registerAttributes() {
		...
		this.getAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue((double)0.32F);
		if (this.isTamed()) {
	...
	public void setTamed(boolean tamed) {
		...
		if (tamed) {
			this.getAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(25.0D);//20
			this.setHealth(25.0F);//20
		} else {
			this.getAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(13.0D);//8
		}

		this.getAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue(5.0D);//4
	}
	...
	public boolean processInteract(PlayerEntity player, Hand hand) {
		...
		if (itemstack.getItem() instanceof SpawnEggItem) {
			...
			return this.isOwner(player) || item == this.getBoneItem() && !this.isAngry();
		} else {
			if (this.isTamed()) {
			...
			} else if (item == this.getBoneItem() && !this.isAngry()) {
				if (!player.abilities.isCreativeMode) {
					itemstack.shrink(1);
				}
	...
	public boolean isBreedingItem(ItemStack stack) {
		...
		return item.isFood() && item.getFood().isMeat() || item == getBoneItem();
	}
	...
	@Override
	public abstract BaseWolfEntity createChild(AgeableEntity parent);

	protected BaseWolfEntity createChild(AgeableEntity parent, BaseWolfEntity child) {
	...
	public boolean canMateWith(AnimalEntity otherAnimal) {
		if (otherAnimal == this) {
			...
			return false;
		} else if (!(otherAnimal instanceof BaseWolfEntity)) {
			return false;
		} else {
	...
	public boolean shouldAttackEntity(LivingEntity target, LivingEntity owner) {
		if (!(target instanceof CreeperEntity) && !(target instanceof GhastEntity)) {
			if (target instanceof BaseWolfEntity) {
				BaseWolfEntity wolfentity = (BaseWolfEntity)target;
	...
	//custom methods van mij
	public Item getBoneItem() {
		return bone;
	}

	public void setBoneItem(Item bone) {
		this.bone = bone;
	}
	...
	class AvoidEntityGoal<T extends LivingEntity> extends net.minecraft.entity.ai.goal.AvoidEntityGoal<T> {
		private final BaseWolfEntity wolf;

		public AvoidEntityGoal(BaseWolfEntity wolfIn, Class<T> entityClassToAvoidIn, float avoidDistanceIn, double farSpeedIn, double nearSpeedIn) {
		...
		private boolean avoidLlama(LlamaEntity llamaIn) {
			return llamaIn.getStrength() >= BaseWolfEntity.this.rand.nextInt(5);
		}
		...
		public void startExecuting() {
			BaseWolfEntity.this.setAttackTarget((LivingEntity)null);
